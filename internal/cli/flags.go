...
type Flags struct {
	...
	Debug              bool
	PGEWebOnly         bool // true when --pge-web-only is set
	EnphaseAPIOnly     bool // true when --enphase-api-only is set
}
...
func ParseFlags() *Flags {
	...
	flag.StringVar(&flags.StartDate, "start-date", "", "Start date (YYYY-MM-DD) for range pulls. Fetches from both Enphase API and PG&E web by default; use --enphase-api-only or --pge-web-only to restrict the source. Skips days already written unless --force is given. Cannot be combined with --continuous, --true-up, or --init.")
